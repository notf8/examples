...
- Фильтрация по диапазону дат/времени: SELECT * from `user` WHERE `reg_date`>= '2019-08-01' and `reg_date`<= '2019-07-01'
    или                                SELECT * from `user` WHERE `reg_date` BETWEEN '2019-08-01' and '2019-07-01' (оператор BEETWEN позволяет установить диапазон между)
- Фильтрация по строкам с помощью масок:  SELECT * from `goods` WHERE `name` LIKE 'Чай' (Оператор LIKE позволяет добавлять в условия маски)
    Маски:
