...
примера, сериалайзер может сам подготовить словарь с ифной по полям модели "Group", а нам не нужно даже циклы писать для этого
...
Документация - https://www.django-rest-framework.org/api-guide/generic-views/#mixins
Важно, стандартные примеси решают все вопросы по CRUD (Create Read Update Delete), потому можно нек выдумывать свои миксины

 - Изменим вьюху GroupsListView в mysite/myapiapp/views.py:
    from rest_framework.generics import GenericAPIView
    from rest_framework.mixins import ListModelMixin

    class GroupsListView(ListModelMixin, GenericAPIView):
        queryset = Group.objects.all()
        serializer_class = GroupSerializer                                  # Тут указываем сериалайзер, который сделали ранее

        def get(self, request: Request) -> Response:
            return self.list(request)

        Так же можно использовать дженерик ListCreateApiView  и тогда даже не нужно переопределять метод get()
