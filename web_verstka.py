...
Ctrl + Shift + P - Окно выбора плагинов
'Live server' - полезный плагин для просмотра изменений кода он-лайн(прямо на сайте)

Плагины для работы:
 - CSS Navigation - https://marketplace.visualstudio.com/items?itemName=pucelle.vscode-css-navigation (зажимаем ctrl и кликаем по классу - открывается окно стилей)
 - eCSStractor - https://marketplace.visualstudio.com/items?itemName=kubosho.ecsstractor (сам копирует все классы из html файла и вставляет в новый файл для css)
...
- Первозданное (изначальное значение цвета базового элемента в браузере)
 ...
 - Текст:
    ...
    - line-height: 30px или 130% (высота строки, межстрочный интервал ) # лучше использовать относительные величины (они зависят от font-size) 130% или 1.3em
...
Стилизация. Прочие свойства:
 - outline - Есть у активных элементов(кнопки, иконки)
    visibility: hidden - визуальное сокрытие объекта, при этом размер за объектом сохраняется (в отличии от display: none)
        объект не убирается из потока
    list-style: none - убирает маркеры в списках
    cursor: pointer - изменяет курсор на рукус пальцем при наведении на объект.
        значение not-allowed (запрещающий знак)
========================================================================================================================

Кодстайл CSS:
 - Почитать - https://codeguide.maxgraph.ru/
 - Описание CSS совйст - 1=позиционирование, 2=совйства блочной модели(размеры, отступы и тд), 3=шрифтовые параметры,
    4=декоративные моменты(box-shadow и тд), 5=транзишены и анимации
 - Стилизация по именам классов - делаем это везде, где только возможно (имя класса должно отражате суть элемента, а не контента)
 - !important - это крайний инструмент (как бейсбольная бита при игре в шашки))) Спользовать его, плохая практика
========================================================================================================================

Подготовка к верстке:
 - Про изображения 1 - https://webref.ru/layout/html5-css3/img/format
 - Про изображения 2 - https://webformyself.com/kakoj-format-izobrazhenij-bolshe-podxodit-vashemu-sajtu/
 - Растровые изображения - Из пикселей (страдает качество при увеличении)
    - Если не важно качество и не нужна прозрачность, выбираем JPEG. Если нет - PNG
    -
 - Векторные изображения(SVG) - качестов не страдает при увеличении. Ипспользуют для иконок, иллюстраций, фигур
========================================================================================================================

FIGMA:
 - Для изменения режима курсора (стрелка/рука), нужно зажать и держать пробел
 - Экспорт графики - выделяем объект, переключаемся на вкладку design, в самомт низу кнопка export

Подключение шрифтов:
Для подключения шрифтов мы используем директиву font-face, а font-family, font-weight, font-style — это свойства подключённого шрифта.
 - Онлайн - fonts.google.com
 - Локально - Создаем папку fonts и в нее копируем скачаные шрифты (нас интересуют только расширения .woff), далее из папки
    stylesheet.css просто копируем стили в наш файл стилей (обязательно ставим их на самый верх). Из скопированных стилей
    удаляем ссылки на файлы .eof  и .ttf. Так же не забыть добавить в url нашу папку fonts
    - Что бы сконвертировать шрифты в woff2, нужен сервис everythingfonts.com (font-face generator) Туда загружаем наши шрифты
        после этого добавляем полученые шрифты в папку fonts и добавляем строки в файл со стилями
        и прописываем параметр - font-display: swap;
        woff - для internet explorer, woff2 - для остальных браузеров
========================================================================================================================

Контейнер сайта (контентная область) - всегда занимает не всю ширину макета сайта
Ширина макета = ширина вью порта пользователя, ширина экрана (всегда не менее 1440p), величина динамическая
Ширина контейнера - величина статическая
Что бы контейнер сайта был адаптивным, нужно добавить параметр max-width: 100%;
