...
Позиционирование: Как например закрепить шапку сайта при скроле
 ...
 - position: absolut - Полностью вырывает элемент из потока, элемент полностью перестает влиять на поток.
   ...
   элемента не будет position: relative или absolut (главное НЕ static), то элемент с position: absolut станет искать след. родителя с relative. Если не найдет,
 ...
 - position: fixed - элементы позиционируются относительно вью-порта браузера (относительно открытого окна браузера), при это не важно
...
Обтекание, скрол и управление слоями:
 ...
 - Добавить скрол внутрь элемента, если текст в него не поместился - overflow: auto; (по умолчанию overflow: visible)
    при значении по умолчанию, тескт просто выходит за границы элемента