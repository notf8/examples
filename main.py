...
Анализ слова 2 (Полиндром). Проверяфем слово полиндром или нет (читается одинково слева направо и наоборот)
# word = input('Введите слово: ')
...
3) По ним(как и по спискам), так же можно брать сроезы: tuple = (1, 2, 3, 4) - tuple[1] покажет 2
...
7) Если в кортеже хранится список (на само деле храниться ссылка на список), то список можно изменить: some_tuple[3][0] = 100
...
с имндексом и со значением, в цикле принято использовать у enumerate. В цикле она перебирает одновременно индекс и значение
...
#     i_scores[i_nums] += 10 # Обращение по индексу, что бы изменить значение
