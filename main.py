...
одлежит погашению равными ежегодными выплатами в конце каждого года,включающими процентные платежи и сумму в погашение
основного долга. После выплаты третьего платежа продлении срока погашения займа на n_2 лет и увеличении процентной ставки
с момента конверсии до i_2%
...
#     if len(string) <= 1: # Т.к. один символ точно палиндром
#         return True
#     if string[0] != string[-1]: # если первый и последний символы не совпали, значит не палиндром
#         return False
...
## abspath - Используется для генерации абсолютных путей (по умолчанию идет от корня системы)
...
Метод .seek()
Используется для указания позиции, с которй читается текстовый файл (фактически, спещаем курсор влево/вправо)
file = open('os_info.txt', 'r', encoding='utf-8')
data1 = file.read(10) # Так указываем до какого символа по счету читаем файл (литерал считается за 1 символ
file.seek(0) # Метод seek указывает номер позиции, с которой начнем чтение (Важно!!! Не работает с кирилицей)
data2 = file.read(22) # Вторым вызовом мы продолжаем с той позиции, на которой остановились в первом вызове, если это
# не требуется, используем метод .seek(1) - описано выше. В скобках сами указываем номер позиции
print(data1)
print(data2)
file.close()
...
Задача 8. Частотный анализ (Гистограмма) В файл analysis.txt выводится доля каждой буквы, встречающейся в тексте, с
тремя знаками в дробной части. Буквы должны быть отсортированы по убыванию их доли. Буквы с равной долей должны
...
#     print("+{:-^19}+".format('+')) # знак ^ в плэйсхолдере означает выравнивание по центру, '<' по левому и  '>' по правому краю
...
Важно! Если после except используется оператор else, то он выполнит код, только если try прошел успешно (без ошибок)
Важно! Оператор finaly будет выполнять код в любом случае, вне зависимости от того, как отработал try (с ошибками или без)
Важно! Exception - ловит абсолютно все ошибки
#num_summ = 0
# nums_count = 0
# try:
...
#
# leeloo = ''      # Нужно вынести пустую переменную за пределы кода, иначе будет вылетать ошибка в конце (т.к. Try ее не пропустит)
# try:
#     leeloo = input_data[4]
#     leeloo = int(leeloo)
# except IndexError:                 # сначала проверяем индекс, потом уже значение (в таком же порядке, как и действия в коде)
...
#
# try:
#     result = int(result) # Тут снова приводим переменную к числу, иначе будет печатать кол-во символов умноженное на 42
...
Задача 3. Счастливое число Что нужно сделать
...
#
# while True:
#     print('\nЧто бы посмотреть текущие сообщения нажми 1, '
#           'что бы отправить сообщение нажми 2.')
#     action = input('Введите 1 или 2: ')
#     if action == '1':
#         try:
#             with open('chat.txt', 'r', encoding='utf-8') as file:
#                 c_history = file.readlines() # Нужно взять переменную, что бы в нее прочесть сразу все строки и работать с ней
#                 print(''.join(c_history))   # На печать из переменной выводим через ''.join (что бы не заводить цикл)
#         except FileNotFoundError:
#             print('В чате пока ничего нет, стань первым!')
#     elif action == '2':
#         message = input('Введите сообщение: ')
#         with open('chat.txt', 'a', encoding='utf-8') as file:
#             file.write('{name} : {message}\n'.format(name=user_name, message=message))
#     else:
#         print('Не корректный ввод. Введите 1 или 2\n')

Нахождение простых чисел. Алгоритм решета эратосфина (сначала удаляются числа после двойки и кратные двум - они составные)
Потом удаляются после тройки и все числа кратные тройке, потом пятерка)
# n = int(input("Введите лимит: "))
# a = [i for i in range(n + 1)]
# print(*a)
# # Будем все составные числа заменять на 0.
#
# a[1] = 0 #Т.к. 1 нам не нужна, сразу заменяем ее на 0
# i = 2 # Цикл начнем с 2
#
# while i <= n ** 0.5: # Перебираем число, пока i меньше половины списка, после прохода половины списка перебирать смысла нет
#     if a[i] != 0: # Если текущая ячейка не занята 0, значит мы встретили простое число
#         j = i ** 2 # Будем вычекркивать из списка кратные i числа. Первое кратное ему число больше i в два раза (6 первое кратное 3)
#         while j <= n:
#             a[j] = 0 #  В ячейку j записываем 0
#             j = j + i # И увеличиваем j на величину i
#     i = i + 1 # Не забываем во внешнем цикле переходить к следующему числу
#
